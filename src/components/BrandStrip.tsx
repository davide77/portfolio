      ...
      <div className="container-lg is-flex is-flex-column has-gap-6">
        <ScrollReveal>
          <div className={"brand-strip__header-block"}>
            <p className={cx("brand-strip__kicker", "is-inline-flex is-align-center has-gap-2 text-xs has-font-semibold uppercase is-primary")}>
          ...
          <ul className="is-flex is-flex-wrap has-gap-3">
            {BRAND_STRIP.names.map((name) => (
              <li key={name} className={"brand-strip__pill-wrap"}>
                <span className={cx("brand-strip__pill", "is-inline-flex is-align-center text-sm has-font-medium has-px-4 has-py-2 is-text")}>
                  {name}
                </span>
              </li>
            ))}
