...
export function ExperienceBlock() {
  return (
    <section className={styles.section} aria-label="About and experience">
        ...
        <ScrollReveal>
          <div className="is-flex is-flex-column has-gap-4">
            ...
            {ABOUT_SECTION.paragraphs.map((p) => (
              <p key={p} className={cx("text-lg leading-relaxed measure-72ch", styles.bodyMuted)}>
                {p}
              </p>
            ))}
          </div>
        ...
        <div className="is-flex is-flex-column has-gap-6">
          <ScrollReveal y={20}>
            <div className="is-flex is-flex-column has-gap-3">
              ...
              <p className={cx("text-lg leading-relaxed measure-72ch", styles.bodyMuted)}>{EXPERIENCE_SECTION.intro}</p>
            </div>
          </ScrollReveal>
          <ul className="is-flex is-flex-column has-gap-6">
            {EXPERIENCE_SECTION.items.map((item, index) => (
              <li key={`${item.org}-${item.period}`} className={styles.item}>
                <ScrollReveal delay={index * 0.1} y={18}>
                  <div>
                    <p className={cx("text-sm has-font-medium", styles.period)}>{item.period}</p>
                    <h3 className={cx("text-xl has-mt-2", styles.heading)}>{item.title}</h3>
                    <p className={cx("text-md has-font-semibold has-mt-1", styles.org)}>{item.org}</p>
                    <p className={cx("has-mt-3 text-base leading-relaxed", styles.bodyMuted)}>{item.summary}</p>
                  </div>
                </ScrollReveal>
              </li>
            ))}
